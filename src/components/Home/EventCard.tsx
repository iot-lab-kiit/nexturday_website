...
export const EventCard = (event: Event) => {
    ...
    return (
        <a href={`/event-details/${event.id}`} className="group relative rounded-2xl overflow-hidden bg-zinc-900/50 backdrop-blur-sm border border-zinc-800/50 hover:bg-zinc-800/50 transition-all duration-300">
            ...
            <div className="px-4 pb-4 flex gap-6">
                <div className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20 backdrop-blur-sm rounded-lg p-3">
                    <div className="text-center">
                        <div className="text-zinc-400 text-sm font-medium">{eventDate.weekday}</div>
                        <div className="text-white font-bold text-2xl">{eventDate.day}</div>
                        <div className="text-zinc-400 text-sm font-medium">{eventDate.month}</div>
                    </div>
                </div>
                ...
                <div className="flex flex-col justify-center space-y-2">
                    ...
                    <div className="space-y-1.5">
                        <p className="text-gray-300 text-sm flex items-center gap-2">
                            {event.details[0].venue ? event.details[0].venue.name : "KIIT Venue"}
                        </p>
