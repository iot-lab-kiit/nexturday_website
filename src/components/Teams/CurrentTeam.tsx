...
import { CheckCircle2, Clock, Copy, Link, Shield, Trash2, UserPlus } from "lucide-react";
...
const CurrentTeam = () => {
    ...
    const handleInviteMember = () => {
        ...
        if (currentTeam && currentTeam.members.length >= currentTeam.maxMembers) {
            toast.error(`Team size limit reached (${currentTeam.maxMembers} members)`);
            return;
        }

        if (currentTeam && newMemberEmail) {
            if (currentTeam.members.some(member => member.email === newMemberEmail)) {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-zinc-400 mt-2 flex items-center gap-2">
                                ...
                                Team Members: {currentTeam?.members.length}/{currentTeam?.maxMembers}
                            </p>
                        </div>
                        ...
                        <div className="flex gap-3 items-center">
                            <motion.button
                                ...
                                onClick={() => {
                                    ...
                                    copyToClipboard(`${window.location.origin}/event-details/${eventId}/teams/team-invite/${currentTeam?.code}`);
                                }}
                ...
                <CardContent className="p-6">
                    <div className="space-y-6">
                        <div className="flex items-center gap-3">
                            <input
                                type="email"
                                placeholder={isTeamFull ? "Team is full" : "Enter member's KIIT email"}
                                value={newMemberEmail}
                                onChange={(e) => setNewMemberEmail(e.target.value)}
                                onKeyPress={handleKeyPress}
                                disabled={isTeamFull}
                                className={`flex-1 p-4 bg-zinc-800/30 rounded-xl border border-zinc-700/50 text-white focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all ${isTeamFull ? 'opacity-50 cursor-not-allowed' : ''}`}
                            />
                            <motion.button
                                ...
                                disabled={isTeamFull}
                                className={`p-4 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-xl shadow-lg shadow-purple-500/20 ${isTeamFull ? 'opacity-50 cursor-not-allowed' : ''}`}
                            >
                                ...
                                {currentTeam?.members.map((member) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            <div className={`p-2 rounded-full ${member.status === 'leader' ? 'bg-purple-500/20' :
                                                member.status === 'accepted' ? 'bg-green-500/20' :
                                                    'bg-yellow-500/20'
                                                }`}>
                                                {member.status === 'leader' ? <Shield className="w-5 h-5 text-purple-400" /> :
                                                    member.status === 'accepted' ? <CheckCircle2 className="w-5 h-5 text-green-400" /> :
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            {currentTeam.leader === useAuthStore.getState().authData.email &&
