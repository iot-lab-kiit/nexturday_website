    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <EventDetailItem
                ...
                label="Time"
                value={currentEvent ? new Date(currentEvent.from).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "---"}
            />
            <EventDetailItem
                icon={<MapPin className="w-5 h-5 text-purple-400" />}
                label="Venue"
                value={currentEvent && currentEvent.details[subEventIndex].venue ? currentEvent.details[subEventIndex].venue.name : "Online"}
            />
        </div>
    )
}
